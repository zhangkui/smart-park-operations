...
func NewRepository() *Repository { return &Repository{items: map[string]Tenant{}} }
func (r *Repository) List() []Tenant {
	r.mu.RLock()
	defer r.mu.RUnlock()
...
func (r *Repository) Get(id string) (Tenant, bool) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	v, ok := r.items[id]
	return v, ok
}
func (r *Repository) Save(v Tenant) Tenant {
	r.mu.Lock()
	defer r.mu.Unlock()
	now := time.Now().UTC()
	if v.ID == "" {
...
func (s *Service) Create(v Tenant) (Tenant, error) {
...
func (s *Service) Update(v Tenant) (Tenant, error) {
...
func (h *Handler) Routes(mux *http.ServeMux) {
	mux.HandleFunc("/api/tenants", h.handleCollection)
	mux.HandleFunc("/api/tenants/", h.handleItem)
}
