...
func NewRepository() *Repository { return &Repository{items: map[string]Alert{}} }
func (r *Repository) List() []Alert {
	r.mu.RLock()
	defer r.mu.RUnlock()
...
func (r *Repository) Get(id string) (Alert, bool) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	v, ok := r.items[id]
	return v, ok
}
func (r *Repository) Save(v Alert) Alert {
	r.mu.Lock()
	defer r.mu.Unlock()
	now := time.Now().UTC()
	if v.ID == "" {
...
func (s *Service) Create(v Alert) (Alert, error) {
...
func (s *Service) Update(v Alert) (Alert, error) {
...
func (h *Handler) Routes(mux *http.ServeMux) {
	mux.HandleFunc("/api/alerts", h.handleCollection)
	mux.HandleFunc("/api/alerts/", h.handleItem)
}
