...
func NewRepository() *Repository { return &Repository{items: map[string]AuditLog{}} }
func (r *Repository) List() []AuditLog {
	r.mu.RLock()
	defer r.mu.RUnlock()
	out := make([]AuditLog, 0, len(r.items))
	for _, item := range r.items {
		out = append(out, item)
	}
	return out
}
func (r *Repository) Get(id string) (AuditLog, bool) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	v, ok := r.items[id]
	return v, ok
}
func (r *Repository) Save(v AuditLog) AuditLog {
	r.mu.Lock()
	defer r.mu.Unlock()
	now := time.Now().UTC()
	if v.ID == "" {
...
func (s *Service) Create(v AuditLog) (AuditLog, error) {
...
func (s *Service) Update(v AuditLog) (AuditLog, error) {
...
func (h *Handler) Routes(mux *http.ServeMux) {
	mux.HandleFunc("/api/audit", h.handleCollection)
	mux.HandleFunc("/api/audit/", h.handleItem)
}
