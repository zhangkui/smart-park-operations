...
type Repository struct {
	mu    sync.RWMutex
	items map[string]Park
}

func NewRepository() *Repository { return &Repository{items: map[string]Park{}} }
func (r *Repository) List() []Park {
	r.mu.RLock()
	defer r.mu.RUnlock()
	out := make([]Park, 0, len(r.items))
	for _, item := range r.items {
		out = append(out, item)
	}
	return out
}
func (r *Repository) Get(id string) (Park, bool) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	v, ok := r.items[id]
	return v, ok
}
func (r *Repository) Save(v Park) Park {
	r.mu.Lock()
	defer r.mu.Unlock()
	now := time.Now().UTC()
	if v.ID == "" {
...
func (s *Service) Create(v Park) (Park, error) {
...
func (s *Service) Update(v Park) (Park, error) {
...
func (h *Handler) Routes(mux *http.ServeMux) {
	mux.HandleFunc("/api/parks", h.handleCollection)
	mux.HandleFunc("/api/parks/", h.handleItem)
}
...
func (h *Handler) handleItem(w http.ResponseWriter, r *http.Request) {
	id := strings.TrimPrefix(r.URL.Path, "/api/parks/")
	if id == "" {
